...
export type ReadPostQuery = {
    ...
    post?: {
        ...
        series?: { __typename?: 'Series'; name?: string | null; url_slug?: string | null } | null
        linked_posts?: {
            __typename?: 'LinkedPosts'
            previous?: { __typename?: 'Post'; id: string; title?: string | null; url_slug?: string | null } | null
            next?: { __typename?: 'Post'; id: string; title?: string | null; url_slug?: string | null } | null
        } | null
    } | null
}
...
export const VelogPostsDocument = gql`
    query velogPosts($cursor: ID, $limit: Int = 20, $username: String!, $temp_only: Boolean, $tag: String) {
        posts(cursor: $cursor, limit: $limit, username: $username, temp_only: $temp_only, tag: $tag) {
            id
            title
            url_slug
        }
...
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
    return {
        ...
        velogPosts(variables: VelogPostsQueryVariables, requestHeaders?: GraphQLClientRequestHeaders, signal?: RequestInit['signal']): Promise<VelogPostsQuery> {
            return withWrapper(
                (wrappedRequestHeaders) => client.request<VelogPostsQuery>({ document: VelogPostsDocument, variables, requestHeaders: { ...requestHeaders, ...wrappedRequestHeaders }, signal }),
                'velogPosts',
                'query',
                variables,
            )
